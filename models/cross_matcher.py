...
class CrossMatch(torch.nn.Module):
    ...
    def forward(self, objects, hints, object_points,object_level_similarity=None,thresh=0.15):
        ...
        if object_level_similarity is not None:
            similarity_score=object_level_similarity.transpose(1,2).max(dim=-1)[0]
            row_min=similarity_score.min(dim=-1)[0].unsqueeze(-1).repeat(1,num_objects)
            row_max_min=(similarity_score.max(dim=-1)[0]-similarity_score.min(dim=-1)[0]).unsqueeze(-1).repeat(1,num_objects)
