...
class CCL(nn.Module):
    ...
    def forward(self, im, s):
        ...
        if self.method == 'log':
            criterion = lambda x: -((1. - x + eps).log()) #distance
        elif self.method == 'tan':
            criterion = lambda x: (x.tan())
        elif self.method == 'abs':
            criterion = lambda x: (x)
        elif self.method == 'exp':
            criterion = lambda x: ((-(1. - x)).exp())
        elif self.method == 'gce':
            criterion = lambda x: ((1. - (1. - x + eps) ** self.q) / self.q)
        else:
...
class CCL_input_score(nn.Module):
    ...
    def forward(self, score, dist):
        ...
        #print(dist.mean())
        if self.method == 'log':
            criterion = lambda x: -((1. - x + eps).log())
        elif self.method == 'tan':
            criterion = lambda x: (x.tan())
        elif self.method == 'abs':
            criterion = lambda x: (x)
