...
def train_epoch(model, dataloader, args, model_coarse=None):
    ...
    for i_batch, batch in pbar:
        ...
        if model_coarse is not None:
            ...
            cell_enc,cell_enc_object_level,cell_enc_relation_level,cell_enc_object_level_mask,cell_enc_relation_level_mask = model_coarse.encode_objects(batch["objects"], batch["object_points"])
            aa=text_enc_object_level
            bb=cell_enc_object_level.transpose(1,2)
            object_level_similarity=torch.matmul(aa,bb)
            output = model(batch["objects"], texts, batch["object_points"],object_level_similarity[:,:,0:args.pad_size],args.confidence_thresh)
        ...
        loss_offsets = criterion_offsets(
            output, torch.tensor(np.array(batch["offsets"]), dtype=torch.float, device=device)
        )
